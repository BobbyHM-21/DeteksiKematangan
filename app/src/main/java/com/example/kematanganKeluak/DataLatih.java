...
public class DataLatih extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_data_latih);
...
//        ((TextView)findViewById(R.id.header)).setText(species.concat(" Detection"));
    ...
    public void openPredictionsPage() {
        if (selectedImageDisplay.getDrawable() == null) {
            ...
            // Convert back to RGB
            Bitmap rgbBitmap = convertToRGB(photo);

            // Add required information to results page intent and start activity
            Intent intent = new Intent(this, ResultDataLatih.class);
    ...
    protected void onActivityResult(int requestCode,
        ...
        if (requestCode == 1) {
            try {
                ...
                Bitmap grayscaleBitmap = convertToGrayscale(img);
                selectedImageDisplay.setImageBitmap(grayscaleBitmap);
                errorText.setText("");
            }
            catch (Exception  e) {
                Log.d("myapp", "error Kamera");
            }
        }
        // Photo Library
    ...
    private Bitmap convertToGrayscale(Bitmap bitmap) {
        ...
        colorMatrix.setSaturation(0); // Mengatur tingkat saturasi menjadi 0 untuk mengubah gambar menjadi grayscale
